...
function recordCoalition(eventId, action, playerName) {
  ...
  return ev[action].length > prev && ev[action].length >= 2; // yeni koalisyon tamamlandı mı
}
...
// Baykuş oyuncusu için: aynı templateId daha önce görüldüyse suspicionLevel bonusu
function checkPatternMemory(player, event) {
  ...
  if (seen.length >= 1) {
    return {
      recognized: true,
      bonus: Math.round(player.suspicionLevel * 0.15), // max ~15 puan
...
function processAction(player, rawEvent, eventForPlayer, action) {
  ...
  // Karar kaydına templateId'yi de ekle (hafıza için)
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (route === "/action" && req.method === "POST") {
      ...
      // BUG FIX #4: strength alanıyla rumor ekle
      if (action === "spread_rumor") {
        kasaba.activeRumors.push({
          id:       `r_${Date.now()}`,
          text:     `${rawEv.title} — kaynak belirsiz`,
          strength: 25 + Math.random() * 35,  // 25-60 arası başlangıç
          trend:    "up",
          origin:   player.name,
        });
      ...
      return json(res, 200, {
        ...
        result: {
          ...
          // BUG FIX #2: Araştır sonucunu istemciye aç
    ...
    // GET /api/trade-info  — Bilgi Ticareti: oyuncu kendi netlik bilgisini satar
    if (route === "/trade-info" && req.method === "GET") {
      ...
      return json(res, 200, {
        ...
        infoLabel: ev.infoLabel,
        perceivedTrust: ev.perceivedTrust,
        price: Math.round(ev.clarity * 120), // satış fiyatı
        seller: player.name,
      });
    }
    ...
    if (route === "/buy-info" && req.method === "POST") {
      ...
      applyMoneyDelta(seller, +Math.round(price * 0.8)); // satıcı %80 alır
      applyArchetypeWeights(seller, { fox: +5 }); // Tilki bonusu
...
server.listen(PORT, () => {
  console.log(`
╔══════════════════════════════════════╗
║   ZOKA v2.0 — http://localhost:${PORT}  ║
║   Kasaba: Yenişehir                  ║
║   Fixes: 4 bug + 5 yeni mekanik      ║
╚══════════════════════════════════════╝`);
});
